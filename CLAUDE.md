...
2. **禁止触碰 ABOT 原厂 `~/abot_ws/`** — 只操作 `~/abot_dev_ws/`
...
# 远端同步（Windows → ABOT）
tar czf - --exclude='__pycache__' --exclude='*.pyc' --exclude='.git' -C <本机仓库> src config scripts | ssh abot@<IP> 'cd ~/abot_dev_ws && tar xzf -'

# 远端单文件同步
scp <本地路径> abot@<IP>:~/abot_dev_ws/<远端路径>

# 远端编译
ssh abot@<IP> 'source /opt/ros/melodic/setup.bash && cd ~/abot_dev_ws && catkin_make'

# 远端启动比赛
ssh abot@<IP> 'bash ~/abot_dev_ws/scripts/competition.sh competition_field true'

# 远端优雅停止（不再需要 pkill 盲杀）
ssh abot@<IP> 'bash ~/abot_dev_ws/scripts/competition.sh --stop'
...
### 两端同步
用户说"两端同步"/"同步"时：
1. 本机是源码唯一源头（git 仓库），远端 `~/abot_dev_ws/` 不是 git 仓库
...
- 若原厂 `~/demo/` 系统在跑，等其结束
- 若需强制清理：`ssh abot@<IP> 'bash ~/abot_dev_ws/scripts/competition.sh --stop'`
