...
2. **禁止触碰 ABOT 原厂 `~/abot_ws/`** — 只操作 `~/PN8RSR/`
...
# 远端同步（Windows → ABOT）
tar czf - --exclude='__pycache__' --exclude='*.pyc' --exclude='.git' -C <本机仓库> src config scripts | ssh abot@<IP> 'cd ~/PN8RSR && tar xzf -'

# 远端单文件同步
scp <本地路径> abot@<IP>:~/PN8RSR/<远端路径>

# 远端编译
ssh abot@<IP> 'source /opt/ros/melodic/setup.bash && cd ~/PN8RSR && catkin_make'

# 远端启动比赛
ssh abot@<IP> 'bash ~/PN8RSR/scripts/competition.sh competition_field true'

# 远端优雅停止（不再需要 pkill 盲杀）
ssh abot@<IP> 'bash ~/PN8RSR/scripts/competition.sh --stop'

# 远端监控
ssh abot@<IP> 'source /opt/ros/melodic/setup.bash && source ~/PN8RSR/devel/setup.bash && rosrun robot_slam nav_monitor.py'
...
- **远端**: ABOT `172.16.25.45`（赛场公用，临时使用）。开发工作空间 `~/PN8RSR/`
...
### 上机
用户说"上机"/"推送"/"远端测试"时：
1. **增量同步**：仅 scp 变更的文件到 ABOT `~/PN8RSR/`（禁止全量 tar，减少耗时）
2. 在远端 `catkin_make` 编译（仅 C++ 修改时需要；纯 Python 修改跳过）
3. Source `~/PN8RSR/devel/setup.bash` 运行
...
scp <本地路径> abot@<IP>:~/PN8RSR/<远端路径>
# 多文件
scp <本地路径1> <本地路径2> abot@<IP>:~/PN8RSR/<远端路径>/
```

### 两端同步
用户说"两端同步"/"同步"时：
1. 本机是源码唯一源头（git 仓库），远端 `~/PN8RSR/` 不是 git 仓库
...
### 下机
用户说"下机"/"远端使用时间结束"时：
1. 将远端 `~/PN8RSR/` 修改文件 `scp` 回本机
2. 确认改动已同步后告知用户
3. ABOT 公用 → 清远端源码（保留 devel/build 免重编译）：`ssh abot@<IP> 'rm -rf ~/PN8RSR/src ~/PN8RSR/config ~/PN8RSR/scripts ~/PN8RSR/launch'`

### 启动前检查
- SSH 到 ABOT，`ps aux | grep ros` 确认无人占用
- 若原厂 `~/demo/` 系统在跑，等其结束
- 若需强制清理：`ssh abot@<IP> 'bash ~/PN8RSR/scripts/competition.sh --stop'`
